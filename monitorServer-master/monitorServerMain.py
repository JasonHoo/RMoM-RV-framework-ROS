...
if __name__ == "__main__":
        ...
        def Launch_Serv():
                ...
                if login == True:
                        t = threading.Thread(target=Recieve_serv,args=())
                        t.start()
        ...
        txtService.insert('insert', "C:\\Users\\Administrator\\Desktop\\Versions\\RMoM-V1.2\\monitorServer-master\\Specification.txt")
      ...
      ' ... ...\n' 
        ...
        def extractAtomP(pDict):
                ...
                for keyAtom in specificDict:
                        if keyAtom != "phi":
                                atompDict[keyAtom] = []
                                find_obj = re.findall('Robot\d{1,3}\.[a-z]\w{1,3}',specificDict[keyAtom])
                                if find_obj:
        ...
        class Handler(BaseRequestHandler):
            def handle(self):
                ...
                while True:
                    ...
                    #print("MainPro:Recieve == "+str(data_rv)+"  >>>>>>> \n")
                    
                    if lastdata != data_rv :
                            if len(data_rv)>0:
                                    
                                if monRunIF==1 or len(lastdata)==0:
                                ...
                                #####完成判定解析，准备输出到界面###########################
                                        ...
                                        if monRunIF==1 and linkOK == 0:
                                                ...
                                                linkOK = 1
                                        if monRunIF==1:
                                                lbverdict.insert(END,TrueRet + " ==>[ " + data_rv + " ]")
                                                #print(data_rv)
                                                lbverdict.see(END)
                                        lastdata = data_rv
